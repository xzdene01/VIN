...
#pragma warning restore 0649
		...
		internal ExposedVariablesEnumerator GetExposedVariablesForType( Type type )
		{
			...
			List<MemberInfo> filteredVariables = new List<MemberInfo>();
			foreach(MemberInfo variable in allVariables)
			{
				if(filter.Contains(variable.Name))
					filteredVariables.Add(variable);
			}
			allVariables = filteredVariables.ToArray();
